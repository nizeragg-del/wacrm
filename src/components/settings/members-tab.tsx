...
const EDITABLE_ROLES: { value: AccountRole; label: string; hint: string }[] = [
  ...
  { value: 'agent', label: 'Atendente', hint: 'Usar funcionalidades; sem configurações' },
...
export function MembersTab() {
  ...
  const loadEverything = useCallback(async () => {
    try {
      ...
      console.error('[MembersTab] erro ao carregar:', err);
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      console.error('[MembersTab] erro ao alterar função:', err);
      toast.error('Não foi possível acessar o servidor');
    } finally {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      console.error('[MembersTab] erro ao remover:', err);
      toast.error('Não foi possível acessar o servidor');
    } finally {
      setPendingMemberAction(null);
    }
  ...
  async function handleRevoke(invite: Invitation) {
    try {
      ...
      console.error('[MembersTab] erro ao revogar:', err);
      toast.error('Não foi possível acessar o servidor');
    }
  }
              ...
              return (
                ...
                >
                  ...
                  <div className="hidden sm:block text-right text-xs text-slate-500">
                    Membro desde {fmtDate(member.joined_at)}
                  </div>
        ...
        <div>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-slate-500">
              A URL do convite em texto simples é mostrada apenas uma vez na criação
