...
const STATUS_OPTIONS: { label: string; value: ConversationStatus; color: string }[] = [
  { label: "Abertas", value: "open", color: "text-primary" },
  { label: "Pendentes", value: "pending", color: "text-amber-400" },
  { label: "Fechadas", value: "closed", color: "text-slate-400" },
];
...
export function MessageThread({
  ...
  const sessionInfo = useMemo(() => {
    ...
    if (expired) {
      return { expired: true, remaining: "Sessão expirada" };
    }
  ...
  const postReaction = useCallback(
    async (messageId: string, emoji: string) => {
      ...
      if (messageId.startsWith("temp-")) {
        toast.error("Aguarde a mensagem ser enviada");
        return;
      }
