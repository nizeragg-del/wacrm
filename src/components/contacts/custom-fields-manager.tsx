  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-slate-700 bg-slate-900 text-slate-200 sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-white">Campos Personalizados</DialogTitle>
...
export function CustomFieldsPanel() {
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        `Excluir "${field.field_name}"? Isso também remove seu valor armazenado em cada contato. Esta ação não pode ser desfeita.`
      )
    ) {
      return;
    }
